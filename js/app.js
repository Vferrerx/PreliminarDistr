...
async function gerarExcel() {
  ...
  try {
    ...
    const result = await buildExcelWorkbook(modelBuffer, allRecords, {});
    state.workbookResult = result;

    const outBuffer = await result.workbook.xlsx.writeBuffer();
    state.excelBlob = new Blob([outBuffer], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
    state.excelFileName = generateFileName(allRecords);

    statusEl.classList.add('ok');
    statusEl.textContent = `Excel gerado com sucesso (${allRecords.length} linha(s)).`;
    $('#downloadFileName').textContent = state.excelFileName;
...
function baixarExcel() {
  ...
  a.href = url;
  a.download = state.excelFileName || 'Programacao_Control_Tower.xlsx';
...
function renderPrefixRefTable(filter) {
  ...
  const rows = PREFIXO_CD_TRANSPORTADORA
    .filter(e => !f || e.prefixo.includes(f) || e.cd.includes(f) || e.transportadora.includes(f))
    .sort((a, b) => a.prefixo.localeCompare(b.prefixo));

  tbody.innerHTML = rows.map(e => `<tr><td>${escapeHtml(e.prefixo)}</td><td>${escapeHtml(e.cd)}</td><td>${escapeHtml(e.transportadora)}</td></tr>`).join('')
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  $('#downloadBtn').addEventListener('click', baixarExcel);
